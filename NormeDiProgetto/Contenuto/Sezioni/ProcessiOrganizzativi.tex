...
\item \textbf{Esperti esterni}: soggetti individuati dall'azienda per richieste specifiche.
\end{itemize}
...
La catalogazione dei rischi si è resa necessaria per assicurare un avanzamento lineare del progetto. Permette al gruppo di individuare in anticipo eventuali situazioni problematiche e sviluppare già da subito un piano di risposta. Tutti i rischi rilevati sono documentati nel \PdP. Il compito di rilevazione dei rischi spetta al \RE.
Vengono individuate le seguenti fasi per la gestione del rischio:
\begin{itemize}
\item Monitoraggio dei rischi già previsti e individuazione di nuove situazioni problematiche;
\item Registrazione di ogni riscontro previsto dai rischi nel \PdP ;
\item Aggiunta di nuovi rischi individuati nel \PdP ;
\item Aggiornamento, se necessario, delle strategie di gestione dei rischi.

\end{itemize}

\subparagraph{Nominazione dei rischi}
Per ogni rischio viene deinito il seguente codice identificativo:
\\
\centerline{\textbf{R[Tipologia][Numero]}}\\
dove:
\begin{itemize}
\item \textbf{Tipologia} intende il tipo di rischio, essi possono essere:
	\begin{itemize}
	\item \textbf{T}: legati alle tecnologie;
	\item \textbf{P}: legati ali membri del gruppo;
	\item \textbf{O}: legati all'organizzazione del lavoro;
	\item \textbf{R}: legati ai requisiti.
	\end{itemize}
\item \textbf{Numero}: rappresenta il numero incrementale identificativo per ogni rischio, parte da 1 e ricomincia da 1 ad ogni cambio di tipologia.
\end{itemize}

\subparagraph{Struttura di un rischio}
Nel \PdP ogni rischio dovrà rispettare la seguente struttura tabellare:
\begin{itemize}
\item \textbf{Codice}: compone la prima riga della tabella; viene fatto seguire da un trattino ("-") e da un breve titolo esplicativo del contenuto della stessa (e.g: RT2 - Problemi software);
\item \textbf{Descrizione}: concisa e sintetica presenta in poche righe il rischio riscontrato;
\item \textbf{Conseguenze}: espone gli effetti a cui il rischio potrebbe portare al lavoro del gruppo e/o al progetto stesso, nel caso non venisse trattato adeguatamente;
\item \textbf{Probabilità di manifestazione}: distinta fra Alta, Medio, Bassa rappresenta la
probabilità che la situazione di rischio si manifesti;
\item \textbf{Pericolosità}: come sopra viene distinta fra Alta, Medio, Bassa; la sua attribuzione
permette di classificare il rischio in base alla pericolosità;
\item \textbf{Precauzioni}: espone le strategie da attuare per cercare di evitare il verificarsi della situazione di rischio;
\item \textbf{Piano di contingenza}: presenta il comportamento da adottare nel caso il rischio si verifichi effettivamente per cercare di limitarne il più possibile le conseguenze negative.
\end{itemize}

\subparagraph{Attualizzazione dei rischi}
Documentare l'attualizzazione dei rischi è egualmente importante, poiché permette di riscontrare i rischi che effettivamente si sono verificati, permettendo di valutare l'efficacia effettiva dei piani di contingenza, e quindi innescare un aggiornamento delle procedure, ma anche della probabilità di manifestazione o pericolosità. \\
L'attuazione viene documentata anch'essa nel \PdQ, e per ogni rischio che si è effettivamente verificato vanno indicati:
\begin{itemize}
\item \textbf{Codice}: compone la prima riga della tabella; viene fatto seguire da un trattino ("-") e da un breve titolo esplicativo del contenuto della stessa (e.g: RT2 - Problemi software);
\item \textbf{Periodo}: i periodi i cui la situazione di rischio si è verificata;
\item \textbf{Descrizione}: concisa e sintetica presenta in poche righe come la situazione di rischio si sia effettivamente verificata;
\item \textbf{Risoluzione}: come sono stati attuati nel concreto i piani di contingenza.
\end{itemize} 
...
Valore che indica la somma di tutte le spese che sono state effettivamente sostenute dal gruppo, questo valore deve essere minore o uguale a quanto preventivato all'interno del consuntivo nel \PdP;
\item \textbf{MPC05 Schedule variance}\\Indica il discostamento percentuale tra la programmazione preventivata e quella effettiva.
La formula è:
\begin{itemize}
